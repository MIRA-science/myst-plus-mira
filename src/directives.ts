...
const directiveRelationshipFields: Partial<
  Record<MiraDirectiveName, RelationshipField[]>
> = {
  claim: ["addresses"],
...
const relationshipOptionDefinitions: Record<
  RelationshipField,
  NonNullable<DirectiveSpec["options"]>[string]
> = {
  addresses: {
    type: String,
    doc: "Comma-separated identifiers of questions this claim addresses.",
  },
  supports: {
    type: String,
    doc: "Comma-separated identifiers of claims this evidence supports.",
  },
  "derived-from": {
    type: String,
    doc: "Comma-separated identifiers of studies this evidence was derived from.",
  },
  produces: {
    type: String,
    doc: "Comma-separated identifiers of evidence this study produces.",
  },
  "modified-by": {
    type: String,
    doc: "Comma-separated identifiers of modifications to the protocol.",
  },
};
...
): Record<string, MiraOptionValue> {
  if (!options) return {};

  return Object.fromEntries(
    Object.entries(options).filter(
      (entry): entry is [string, MiraOptionValue] => {
        const [, value] = entry;
...
function createMiraDirective(name: MiraDirectiveName): DirectiveSpec {
  ...
  const extraOptions = Object.fromEntries(
    relationshipFields.map((field) => [
      field,
      relationshipOptionDefinitions[field],
    ]),
  );
...
export const miraDirectives: DirectiveSpec[] =
  miraDirectiveNames.map(createMiraDirective);

export function isMiraDirectiveNode(
  node: GenericNode,
): node is MiraDirectiveNode {
